...
int main(int argc, char **argv)
{
    ...
    char *sblk, *cblk, *chks;
    int seof, ceof;
    int srcerr = 0, chkerr, bit_errs;
    int i, n;
    FILE *srcf = NULL, *codef;
    ...
    if (gen_file != 0)
    {
        gen_matrix = ldpc_generator_read(gen_file, 1, 0, H);
        M = gen_matrix->M;
        N = gen_matrix->N;
    }
    ...
    for (n = 0;; n++)
    {
        ...
        if (gen_file != 0)
        {
            ...
            if (source_file != 0 && !seof)
            {
                for (i = M; i < N; i++)
                {
                    if (cblk[gen_matrix->cols[i]] != sblk[i - M])
                    {
                        srcerr += 1;
                    }
                }
