...
rs_codec* rs_create(int message_size, int parity_size, uint32_t prime_plonomial)
{
    ...
    //rs->generator_polynomial = BOXING_MEMORY_ALLOCATE_TYPE_ARRAY(uint32_t, parity_size * 4);
    ...
    uint32_t *syndrome_bytes)
{
    ...
    for (n = 0; n < parity_size; n++)
    {
        ...
        //Polynomial_utils::mulz(D, m_maximun_degree_polynomials);
        for (int i = parity_size * 2 - 1; i > 0; i--) D[i] = D[i - 1];
        D[0] = 0;
    }
