...
//  PROJECT INCLUDES
//
#include    "boxing/frame/frametrackergpf_1.h"
...
boxing_tracker_gpf_1 * boxing_frame_tracker_gpf_1_create(boxing_frame_gpf_1 * generic_frame)
{
    ...
    tracker->base.base.free = frame_tracker_gpf_1_free;
    tracker->base.base.type = FRAME_TRACKER_GPF1_NAME;
    ...
    int num_syncpoints_h = (container_dimension.x - 2*generic_frame->sync_point_radius - 1) / generic_frame->sync_point_h_distance + 1;
    int syncpoint_offset_h = generic_frame->sync_point_h_offset;
    if(syncpoint_offset_h < 0)
    {
        syncpoint_offset_h = (container_dimension.x-((num_syncpoints_h-1)*generic_frame->sync_point_h_distance+1))/2;
    }
    int num_syncpoints_v = (container_dimension.y - 2*generic_frame->sync_point_radius - 1) / generic_frame->sync_point_v_distance + 1;
    int syncpoint_offset_v = generic_frame->sync_point_v_offset;
    if(syncpoint_offset_v < 0)
    {
...
static int track_frame_analog_mode(boxing_tracker_gpf * tracker, const boxing_image8 * input_image)
{
    ...
    *SMEMBER(bottom_right_corner_mark_sampler)->location_matrix.data = boxing_math_pointi_to_float( &corner_marks.bottom_right );
    ...
    if(tracker->base.mode & BOXING_TRACK_CALIBRATION_BAR)
    {
        ...
        boxing_pointf point = boxing_coordmapper_map(&SMEMBER(coordinate_mapper), location.x, location.y+size.y/2); // todo ask if conversion to float is ok
...
static gvector * track_reference_bar_sync(boxing_sampler * sampler, const boxing_image8 * image, int sync_distance, int sync_offset)
{
    ...
    gvector * sync_locations = gvector_create(sizeof(sync_location), 3 + ((reference_bar->size - sync_offset) / sync_distance));
    ...
    int i = 1;
	char sync_symbol[] = { 1, 0, 1, 0, 0, 0, 1, 0, 1 };

    for (; (i < (int)reference_bar->size - 4) && (sync_count < (sync_locations->size - 4)); i++)
    {
        int sum = 0;
    ...
    const frame_corner_marks* corner_marks )
{
    ...
    for(int i = 0; i < vertical_tile_count; i++)    
    {
        ...
        metadata_left_bar_points[i].y =  corner_marks->bottom_left.y + BASEMEMBER(base.y_sampling_rate) * (tile_size.y / 2 + tile_size.y * i);
        metadata_right_bar_points[i].x = (boxing_float)corner_marks->bottom_right.x;
        metadata_right_bar_points[i].y = corner_marks->bottom_right.y + BASEMEMBER(base.y_sampling_rate) * (tile_size.y / 2 + tile_size.y * i);
    }
