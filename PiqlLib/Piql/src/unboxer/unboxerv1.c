...
static void         dunboxerv1_calculate_lut(struct boxing_tracker_s * tracker, const boxing_image8 * image, boxing_image8 ** lut);
static void         dunboxerv1_apply_lut(boxing_image8 * image, const boxing_image8 * lut);
    ...
    void * user_data)
{
    ...
    boxing_frame_tracker_destroy(tracker);
...
static int dunboxerv1_decode_step(boxing_dunboxerv1 * unboxer, gvector * data, boxing_metadata_list * metadata, boxing_stats_decode * decode_stats, unsigned int step, void * user_data)
{
    ...
    if( step < the_dispatcher->encode_codecs.size)
    {
        ...
        retval = boxing_codecdispatcher_decode_step_codec(codec, data, erasures, decode_stats, user_data) ? BOXING_UNBOXER_OK : BOXING_UNBOXER_DATA_DECODE_ERROR;
...
static void dunboxerv1_calculate_lut(struct boxing_tracker_s * tracker, const boxing_image8 * image, boxing_image8** lut)
{
    ...
    const unsigned int lut_size = calibration_bar_image->width * calibration_bar_image->height;
    // Search for max value
    boxing_image8_pixel max_pixel_value = BOXING_PIXEL_MIN;
    for (unsigned int i = 0; i < lut_size; ++i)
    {
        const boxing_image8_pixel pixel = calibration_bar_image->data[i];
        if (max_pixel_value < pixel)
        {
    ...
    {
        const boxing_image8_pixel pixel = calibration_bar_image->data[i];
        calibration_bar_image->data[i] = BOXING_PIXEL_MAX * pixel / max_pixel_value;
    }

    *lut = calibration_bar_image;
}

static void dunboxerv1_apply_lut(boxing_image8 * image, const boxing_image8 * lut)
{
    const size_t image_size = image->width * image->height;
    boxing_image8_pixel max_pixel_density = BOXING_PIXEL_MIN;
    for (size_t i = 0; i < image_size; ++i)
    {
        const boxing_image8_pixel pixel = image->data[i];
        if (max_pixel_density < pixel)
        {
            max_pixel_density = pixel;
        }
    }
    for (size_t i = 0; i < image_size; ++i)
    {
        const boxing_image8_pixel pixel = BOXING_PIXEL_MAX * image->data[i] / max_pixel_density;
        image->data[i] = lut->data[pixel];
    }
}
    ...
    void* user_data)
{
    ...
    gvector * metadata_bytes = extract_metadata_bytes(samples);
    ...
    DBOOL quantize_data)
{
    ...
    {
        tracker_mode |= BOXING_SIMULATED_TRACKING;
    }
    else
    {
        tracker_mode &= ~BOXING_SIMULATED_TRACKING;
    ...
    // Extract metadata
    boxing_sampler * metadata_sampler = boxing_frame_tracker_get_container_sampler(tracker, BOXING_SAMPLE_CONTAINER_METADATA);
    ...
    {
        DLOG_WARNING1("unboxing_load_data_from_image:  Meta data doesn't contain \"MetaDataType::SymbolsPerPixel\", using default instead: %d", symbols_per_pixel);
    }
    ...
    boxing_image8 * lut = NULL;
    if (is_analogue_data)
    {
        dunboxerv1_calculate_lut(tracker, image, &lut);
    }

    // Extract content
    boxing_sampler * content_sampler = boxing_frame_tracker_get_container_sampler(tracker, BOXING_SAMPLE_CONTAINER_CONTENT);
    if(!content_sampler || !content_sampler->state)
    {
        DLOG_ERROR( "unboxing_load_data_from_image:  container data locations not vaild");
        if (lut)
        {
            boxing_image8_free(lut);
        }
