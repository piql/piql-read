...
//---------------------------------------------------------------------------- 
/*!
 *  \brief GFP 1 default implementation of @ref boxing_syncpoints_correct_frame_geometry_cb. This 
 *  function uses the sync points at given locations to compensate for
    ...
    const boxing_matrixi *sync_point_location_index, const correct_frame_geometry_properties * properties)
{
    ...
    boxing_matrixf * sync_point_locations_from_scan_image = determine_syncpoints(&sync_point_locations_from_raster_locations, frame, properties->sync_point_search_radius, properties->sync_point_center_is_bright);

    // Calculate offset
    boxing_matrixf * sync_point_offsets = sync_point_locations_from_scan_image;
    ...
    for (int iy = 0; iy < use_rows; iy++)
    {
        ...
        {
            boxing_pointf *to = sync_point_offsets->data + ix + iy * sync_point_offsets->width;
    ...
    for (int iy = 0; iy < use_rows; iy++)
    {
        for (int ix = 0; ix < use_cols; ix++)
        {
            boxing_pointf *to = sync_point_locations_from_raster_locations.data + ix + iy * sync_point_locations_from_raster_locations.width;
...
static void correct_symbol_location_matrix_area(boxing_matrixf * initial_symbol_location_matrix, const boxing_recti *ref_area_src, const rectanglef *offset, DBOOL correct_right_border, DBOOL correct_bottom_border)
{
    ...
    for (unsigned int iy = 0; iy < rect_height; iy++)
    {
        ...
        for (unsigned int ix = 0; ix < rect_width; ix++)
        {
            ...
            boxing_pointf *element = &MATRIX_ELEMENT(initial_symbol_location_matrix, y, ref_area_src_x + ix);
