...
//---------------------------------------------------------------------------- 
/*! 
 *  \ingroup frametrackerutil
 *  \brief   Track reference bar
 *
 *  \param image                Image to be tracked
 *  \param sample_start         Start coordinate for tracking
 *  \param sample_end           End coordinate for tracking
 *  \param bar_points           Positions all peaks and valleys in the reference bar signal.
 *  \param bar_points_size      Number of positions in reference bar points.
 *  \param reference_point      Expected position of first sample.
 *  \param perpendicular_samples Width of sample in pixels, perpendicular to sample line. 
 *
 ...
 *  \param[in]  border    Cooridinate of border (if found)
 *  \return true if border is found. 
 */ 
...
DBOOL boxing_frame_tracker_util_find_horizontal_border(const boxing_image8 * image, const boxing_pointi *from, int lenght, boxing_pointi *border)
{
    ...
    if(start_y < stop_y)
    {
        boxing_pointi point_dim = {rectangle_size, lenght};
...
static DBOOL track_reference_bar_location(gvector * samples, boxing_double * locations, unsigned int locations_size, boxing_float sampling_rate, boxing_float reference_point)
{
    ...
    boxing_float * filter_coeffsicients = BOXING_MEMORY_ALLOCATE_TYPE_ARRAY(boxing_float, filter_order);
    boxing_dsp_low_pass_filter(0.41f, sampling_rate, filter_coeffsicients, filter_order);
    ...
    boxing_dsp_filtfilt(energy, (unsigned int)samples->size, energy, (unsigned int)samples->size, filter_coeffsicients, filter_order);
    ...
    boxing_memory_free(filter_coeffsicients);
